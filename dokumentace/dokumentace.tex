...
RSA komunikuje se scannerem - volá \texttt{getToken} na zisk tokenu, a PA - volá ji pomocí\\
\texttt{expressionAnalysis} s parametry jedním až dvěma tokeny, které načetla, avšak patří PA, a tokenem \texttt{res}, který nese jméno, které má PA použít pro výsledek.
...
Při nalezení syntaktické (popř. při všech chybách kromě vnitřní nebo lexikální) kontrola nekončí, v pravidlech se nachází typy tokenů, na kterých se pokusí zotavit (např. \texttt{EOL} nebo \texttt{(}, pokud RSA najde chybu, pokusí se načítat tokeny tak dlouho, dokud na tento token nenarazí, a poté pokračuje).
